#!/bin/bash
set -euo pipefail

CLIENT_NAME="${1:-}"
if [[ -z "$CLIENT_NAME" ]] || [[ ! "$CLIENT_NAME" =~ ^[a-zA-Z0-9_-]+$ ]] || [[ ${#CLIENT_NAME} -gt 15 ]]; then
  echo "Usage: $0 <client_name>" >&2
  echo "Client name: alphanumeric, underscores/dashes, max 15 chars."
  exit 1
fi

WG_CONF="/config/wg_confs/wg0.conf"
CLIENTS_DIR="/config/clients"
TEMPLATE="/config/templates/peer.conf"

[[ -f "$WG_CONF" ]] || { echo "❌ $WG_CONF not found. Wait for container initialization." >&2; exit 1; }
[[ -f "$TEMPLATE" ]] || { echo "❌ $TEMPLATE not found." >&2; exit 1; }

SERVER_PRIV_KEY=$(grep -oP 'PrivateKey = \K.*' "$WG_CONF")
SERVER_PUB_KEY=$(wg pubkey <<< "$SERVER_PRIV_KEY")
SERVER_PORT=$(grep -oP 'ListenPort = \K.*' "$WG_CONF")
SERVER_WG_IPV4=$(grep -oP 'Address = \K[^,/]*' "$WG_CONF")

grep -q "^### Client ${CLIENT_NAME}\$" "$WG_CONF" && { echo "⚠️ Client '$CLIENT_NAME' already exists." >&2; exit 1; }

BASE_IPV4=$(echo "$SERVER_WG_IPV4" | cut -d'.' -f1-3)
CLIENT_WG_IPV4=""
for i in {2..254}; do
  IP="${BASE_IPV4}.${i}"
  if ! grep -q "${IP}/32" "$WG_CONF"; then
    CLIENT_WG_IPV4="$IP"
    break
  fi
done
[[ -z "$CLIENT_WG_IPV4" ]] && { echo "❌ No free IPv4 addresses left." >&2; exit 1; }

CLIENT_PRIV_KEY=$(wg genkey)
CLIENT_PUB_KEY=$(wg pubkey <<< "$CLIENT_PRIV_KEY")
PRESHARED_KEY=$(wg genpsk)

CLIENT_FILES_DIR="$CLIENTS_DIR/${CLIENT_NAME}"
mkdir -p "$CLIENT_FILES_DIR"

echo "$CLIENT_PRIV_KEY" > "$CLIENT_FILES_DIR/privatekey"
echo "$CLIENT_PUB_KEY" > "$CLIENT_FILES_DIR/publickey"
echo "$PRESHARED_KEY" > "$CLIENT_FILES_DIR/presharedkey"
echo "$SERVER_PUB_KEY" > "$CLIENT_FILES_DIR/server-publickey"

export CLIENT_IP="${CLIENT_WG_IPV4}/32"
export PEER_ID="$CLIENT_NAME"
export PEERDNS="${PEERDNS:-8.8.8.8}"
export SERVERURL="${SERVERURL}"
export SERVERPORT="${SERVERPORT}"
export ALLOWEDIPS="0.0.0.0/0,::/0"
export CLIENT_PRIVATE_KEY="$CLIENT_PRIV_KEY"
export SERVER_PUBLIC_KEY="$SERVER_PUB_KEY"
export PRESHARED_KEY="$PRESHARED_KEY"

CLIENT_CONFIG_CONTENT=$(sed \
  -e "s|\${CLIENT_IP}|$CLIENT_IP|g" \
  -e "s|\${PEER_ID}|$PEER_ID|g" \
  -e "s|\${PEERDNS}|$PEERDNS|g" \
  -e "s|\${SERVERURL}|$SERVERURL|g" \
  -e "s|\${SERVERPORT}|$SERVERPORT|g" \
  -e "s|\${ALLOWEDIPS}|$ALLOWEDIPS|g" \
  -e "s|\${CLIENT_PRIVATE_KEY}|$CLIENT_PRIVATE_KEY|g" \
  -e "s|\${SERVER_PUBLIC_KEY}|$SERVER_PUBLIC_KEY|g" \
  -e "s|\${PRESHARED_KEY}|$PRESHARED_KEY|g" \
  "$TEMPLATE")

IP_WITHOUT_DOTS=$(echo "$CLIENT_WG_IPV4" | tr -d '.')
CONFIG_FILENAME="wg-${IP_WITHOUT_DOTS}-${CLIENT_NAME}.conf"

echo "$CLIENT_CONFIG_CONTENT" > "$CLIENT_FILES_DIR/${CONFIG_FILENAME}"
ln -sf "$CLIENT_FILES_DIR/${CONFIG_FILENAME}" "$CLIENTS_DIR/${CONFIG_FILENAME}"

cat >> "$WG_CONF" <<EOF

### Client $CLIENT_NAME
[Peer]
PublicKey = $CLIENT_PUB_KEY
PresharedKey = $PRESHARED_KEY
AllowedIPs = $CLIENT_WG_IPV4/32
EOF

wg syncconf wg0 <(wg-quick strip wg0) 2>/dev/null || true

echo "✅ Client '$CLIENT_NAME' created!"
echo "📄 Config: $CLIENT_FILES_DIR/${CONFIG_FILENAME}"
qrencode -t ansiutf8 <<< "$CLIENT_CONFIG_CONTENT" 2>/dev/null || echo "⚠️ QR skipped (qrencode not found)"